...
//					System.out.println(newName.getText());
	...
	private void insertData(Inshurance_companyData rc) {
		...
		try {
			...
			Connector.a.connectDB();
			String sql = "Insert into inshurance_company (inshurance_companyName, inshurance_companyDiscount, numberOfCustomer)"
					+ " values(?,?,?)";
	...
	void insertOnAction(ActionEvent event) {
		...
		try {
			...
			rc = new Inshurance_companyData(addName.getText(), Integer.parseInt(addDiscount.getText()),
					Integer.parseInt(addCustomerNumber.getText()));
			dataList.add(rc);
			insertData(rc);
			initialize();
			addName.clear();
